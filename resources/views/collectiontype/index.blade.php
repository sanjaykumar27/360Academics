...
<?php
 ...
 ?>
<div class="d-wrapper d-master" ng-controller="collectiontypeController" ng-cloak >
       ...
       <div class="row clearfix">
         <div class="col-xs-12 col-sm-5 col-md-4 col-lg-4">
           <ul class="d-breadcrum">
             ...
             <li><span>Collection Type</span></li>
           </ul>
         </div> 
       </div>
     </div>
       ...
       <div class="container" ng-show = "list.length > 0">
         <md-toolbar class="d-listing_toolbar">
            <div class="d-searchbar" ng-show="showme"> 
              <button ng-click="showme=false" class="md-fab md-mini md-button md-ink-ripple" aria-label="Search Collection Type">
             ...
             <tbody>
               <tr ng-repeat="data in filtered = (list | filter:search ) |  startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit">
				...
				<?php //if(in_array('Collection Type Update',$PermissionFunction)){ ?>
				  ...
				  <!--<a href="{{url('')}}/collection?id=@{{data.id}}"></a>-->
		<md-button aria-label="collection" href="{{url('/collection')}}?id=@{{data.id}}" class="md-fab md-mini md-raised md-mini">
		   ...
		   <div class="modal-body clearfix">
			  <div class="panel panel-default">
				<div class="panel-heading clearfix">
				  <h3 class="panel-title d-no_padding">@{{collectiontype_form}} Collection Type</h3> 
				</div> 
			  </div>
			  <div class="clearfix well">
			  <form   method="post" name="frmcollectiontype" id="frmcollectiontype" ng-submit="saverecord(frmcollectiontype.$valid)"  novalidate="novalidate" >
			  <input type="hidden" name="_token"  value="{{csrf_token()}}" ng-model="singledata._token">
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12"> 
					<md-input-container class="md-block" ng-class="(submitted && !frmcollectiontype.collectiontype.$valid) ? 'd-validation-error' : ''" flex-gt-sm> 
					  <label>Collection Type<span> *</span></label>
					  <input type="text" ng-model="singledata.collectiontype" name="collectiontype" required ng-minlength="3" ng-model-options="{debounce : 700}">
					  <div ng-show="submitted && frmcollectiontype.collectiontype.$error " class="md-input-messages-animation md-auto-hide ng-active">
						 <div ng-show="frmcollectiontype.collectiontype.$error.required">{{ config('constant.error.Required') }}</div>
						 <div ng-show="frmcollectiontype.collectiontype.$error.minlength">{{ config('constant.error.CollectionTypeMinLength') }}</div>
					  </div>
					</md-input-container>
				</div>
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
